...
import { Star, MapPin, Search, Filter, RefreshCw, Heart } from "lucide-react"
import { useState, useCallback, useMemo, useEffect } from "react"
import { useSearchParams, useRouter, usePathname } from "next/navigation"
import { useAuth } from "@/components/auth-provider"

function SportsSubcategoryFilter({ subcategories, selected, onChange }: { subcategories: any[]; selected: string; onChange: (s: string) => void }) {
  return (
...
function SportsItemCard({ item }: { item: any }) {
...
export default function SportsPage() {
  ...
  const urlPrice = urlPriceParam ? parseInt(urlPriceParam) : 2000
  ...
  const subcategories = [
    ...
    { name:"Football Shoes", value:"football-shoes" },
  ]
  const brandsList = ["Adidas","Nike","Yonex","Nivia","SG","GM","Cosco","Spalding","Wilson"]
  const cities = [
  ...
  const sportsItems = allItems.filter(item => item.category === "Sports")

  const updateURL = useCallback(
    (params: Record<string, string>) => {
      const current = new URLSearchParams()
      Object.entries(params).forEach(([key, value]) => {
        if (value && value !== "all" && value !== "" && value !== "2000" && value !== "recommended") {
  ...
  const filteredAndSortedItems = useMemo(() => {
    const filtered = sportsItems.filter((item) => {
      const subcategoryMatch = sub === "all" || (item.subcategory && item.subcategory.toLowerCase() === sub.toLowerCase())
      ...
      const searchMatch = !q || 
        ...
        (item.specifications?.Brand && item.specifications.Brand.toLowerCase().includes(q.toLowerCase()))
      const ratingMatch = rating === "any" || (item.owner_rating ?? 0) >= parseFloat(rating)
      const brandMatch = brands.length === 0 || (item.specifications?.Brand && brands.includes(item.specifications.Brand))
    ...
    return sorted
  }, [sportsItems, sub, loc, price, q, rating, brands, sortBy])
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">Sports Rentals</h1>
          <p className="text-muted-foreground">
            Rent sports equipment and gear for every game
          </p>
        </div>
        <div className="mb-8">
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1">
              <Input
                name="searchQuery"
                placeholder="Search sports equipment (football, bat, cycle...)"
                value={q}
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
              <div>
                <span className="text-lg font-medium">
                  {filteredAndSortedItems.length} sports items
                  {sub !== "all" && (
                    <span className="ml-2">
            ...
            {filteredAndSortedItems.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredAndSortedItems.map((item) => (
                  <SportsItemCard key={item.id} item={item} />
                ))}
              </div>
            ) : (
              <div className="text-center py-12">
                <p className="text-lg text-muted-foreground mb-4">
                  No sports items found matching your criteria
                </p>
